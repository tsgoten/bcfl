...
//   });
  ...
  it("Should edure stress under real models", async function () {
    const model_structure_large = [62002]
    const model_parameters = same_parameters_factory(model_structure_large, 1);
    const federator = await deploy_federator(model_structure_large, model_parameters);
    const weights_to_update = [
      2, 3
    ];
    weights_to_update.forEach(async (weight) => {
      const parameters = same_parameters_factory(model_structure_large, weight);
      await federator.update(parameters);
    });

    expected_parameters = expected_aggregated_model(
      model_structure_large,
      1,
      weights_to_update,
      1
    );
